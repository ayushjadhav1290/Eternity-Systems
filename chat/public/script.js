document.addEventListener('DOMContentLoaded', () => {
    ...
    /**
     * Resets the entire conversation to start fresh.
     */
    function resetConversation() {
        // Clear all chat messages
        chatMessages.innerHTML = '';
        
        // Reset criteria widget
        if (criteriaWidget) {
            criteriaWidget.classList.add('hidden');
        ...
        // Add welcome message
        addMessage("Hello! I can help you find the perfect cloud provider. Tell me what you are looking for (e.g., \"I need something cheap and reliable for a startup\").", 'system');
    ...
    function startAnalysisTimer() {
        // Timer disabled - results only show when user clicks "Find Best Provider" button
...
*/
    ...
    prevSlideBtn.addEventListener('click', () => {
        if (currentSlideId === 'slide-details') {
            showSlide('slide-results');
        } else if (currentSlideId === 'slide-results') {
    ...
    if (!themeToggleBtn) {
        ...
        if (header) {
            ...
            themeToggleBtn.innerHTML = `<svg class="theme-icon-moon" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z"></path></svg><svg class="theme-icon-sun" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z"></path></svg>`;
            header.appendChild(themeToggleBtn);
        }
    }

    // Remove specific text requested
    const textToRemove = "Find the best cloud provider for your needs.";
    const potentialElements = document.querySelectorAll('h1, h2, h3, p, div, span');
    potentialElements.forEach(el => {
        if (el.children.length === 0 && el.textContent.trim() === textToRemove) {
            el.remove();
        }
    });
